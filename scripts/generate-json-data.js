#!/usr/bin/env node
// scripts/generate-json-data.js
// One-time script: converts idCsvData + egoData from data.js into
// data/identities.json and data/egos.json.
// Run from project root: node scripts/generate-json-data.js

const fs = require('fs');
const path = require('path');

// ─── Slug helpers (mirrors server/utils/idData.js) ────────────────────────────
...
// ─── Raw data (copied verbatim from data.js) ──────────────────────────────────
...
"Shi Assoc. East Section 3 Faust","Bleed,Poise","Wrath,Pride,Lust","000"
"The Index Proselyte:【Paper Slip】Faust","Poise,Sinking","Sloth,Pride,Gloom","000"
...
"The Index Proxy - Effloresced E.G.O::Procuration Don Quixote","Poise,Sinking","Lust,Pride,Gloom","000"
...
"W Corp. L4 Cleanup Agent - CCA Heathcliff","Charge,Rupture","Gluttony,Gloom,Envy","000"
"Heishou Pack - You Branch Adept Heathcliff","Burn,Rupture","Envy,Gluttony,Wrath","000"
...
"R Corp. 4th Pack Reindeer Rodion","Charge,Sinking","Gloom,Envy,Wrath","000"
...
"LCA Udjat Vanguard Team 3 Leader Outis","Tremor,Sinking","Pride,Sloth,Gloom","000"
...
const egoRawData = `Crow's Eye View Yi Sang - ZAYIN - Sloth - Yellow
...
Ardor Blossom Star Faust - HE - Wrath - Red
...
Great Trichiliocosm [三千大世界] Ryōshū - WAW - Wrath - Red
...
Move-in Reg. Heathcliff - TETH - Envy - Purple
...
const koreanEgoNames = {
    "Crow's Eye View": "오감도", "Bygone Days": "지난 날", "4th Match Flame": "4번째 성냥불",
    "Wishing Cairn": "소망석", "Dimension Shredder": "차원찢개", "Fell Bullet": "흉탄",
    "Sunshower": "여우비", "Representation Emitter": "표상 방출기", "Hex Nail": "저주못",
    "9:2": "9장 2절", "Lasso": "올가미", "Fluid Sac": "물주머니", "Telepole": "전봇대",
    "Thoracalgia": "흉통", "Everlasting": "영속", "La Sangre de Sancho": "라 샹그레 데 산쵸",
    "Lifetime Stew": "평생 스튜", "Electric Screaming": "전기울음", "Red Sheet": "홍적",
    "Yearning-Mircalla": "갈망 - 미르칼라", "In the Name of Love and Hate": "사랑과 증오의 이름으로",
    "Forest for the Flames": "삼라염상", "Soda": "소다", "Red Eyes": "적안",
    "Blind Obsession": "맹목", "Red Eyes (Open)": "적안(開)", "Contempt, Awe": "경멸, 경외",
    "Chains of Others": "타인의 사슬", "Screwloose Wallop": "나사빠진 일격", "Regret": "후회",
    "Pursuance": "집행", "Capote": "카포테", "Crushbound Past": "분쇄될 과거",
    "Land of Illusion": "허환경", "Roseate Desire": "분홍욕망", "Cavernous Wailing": "낮은울음",
    "Effervescent Corrosion": "들끓는 부식", "Tears of the Tarnished Blood [汚血泣淚]": "오혈읍루 [汚血泣淚]",
    "Bodysack": "시체자루", "Holiday": "홀리데이", "AEDD": "AEDD",
    ...
    "Legerdemain": "눈속임", "Solemn Lament": "엄숙한 애도", "Garden of Thorns": "가시 화원",
    // Missing translations (left empty intentionally):
    "Command : Meltdown": "", "To Remain Oneself [宁作吾]": "", "Tidal Elegy": "", "Unbrilliant Glory": "",
    "Ardor Blossom Star": "", "Great Trichiliocosm [三千大世界]": "", "Move-in Reg.": "", "Harmony": "",
    "I'll Go fer Scissors. How 'Bout You?": "",
};
...
function parseIDCSV(csv) {
    ...
    for (let i = 1; i < lines.length; i++) {
        ...
        result.push({
            name: obj.Name,
            keywords: obj.Keywords ? obj.Keywords.split(',').map(k => k.trim()).filter(Boolean) : [],
            sinAffinities: obj.SinAffinities ? obj.SinAffinities.split(',').map(s => s.trim()).filter(Boolean) : [],
            rarity: obj.Rarity,
        });
    }
    return result;
...
// Reads existing JSON, merges in any new entries from the raw data above,
// never removes entries that exist in the JSON but not in raw data.
const outDir = path.join(__dirname, '../data');
const idJsonPath  = path.join(outDir, 'identities.json');
const egoJsonPath = path.join(outDir, 'egos.json');

// Load existing JSON (or start fresh if not present)
const existingIds  = fs.existsSync(idJsonPath)  ? JSON.parse(fs.readFileSync(idJsonPath,  'utf8')) : [];
const existingEgos = fs.existsSync(egoJsonPath) ? JSON.parse(fs.readFileSync(egoJsonPath, 'utf8')) : [];

// Build lookup sets to detect duplicates
const idKey  = entry => entry.name;
const egoKey = entry => `${entry.name}|||${entry.sinner}`;
const existingIdKeys  = new Set(existingIds.map(idKey));
const existingEgoKeys = new Set(existingEgos.map(egoKey));

// Parse from raw data, then append only genuinely new entries
const rawIds  = parseIDCSV(idCsvData);
const rawEgos = parseEGOData(egoRawData);

const newIds  = rawIds.filter(e => !existingIdKeys.has(idKey(e)));
const newEgos = rawEgos.filter(e => !existingEgoKeys.has(egoKey(e)));

const mergedIds  = [...existingIds,  ...newIds];
const mergedEgos = [...existingEgos, ...newEgos];

fs.writeFileSync(idJsonPath,  JSON.stringify(mergedIds,  null, 2), 'utf8');
fs.writeFileSync(egoJsonPath, JSON.stringify(mergedEgos, null, 2), 'utf8');

if (newIds.length)  console.log(`✓ Added ${newIds.length} new identities → data/identities.json (${mergedIds.length} total)`);
else                console.log(`✓ data/identities.json unchanged (${mergedIds.length} entries)`);
if (newEgos.length) console.log(`✓ Added ${newEgos.length} new EGOs → data/egos.json (${mergedEgos.length} total)`);
else                console.log(`✓ data/egos.json unchanged (${mergedEgos.length} entries)`);

// Sanity check: flag any EGO whose name wasn't found in Korean map at all
const unmapped = mergedEgos.filter(e => !(e.name in koreanEgoNames));
if (unmapped.length) {
    console.warn(`\nWARN: ${unmapped.length} EGO names not in koreanEgoNames map:`);
    unmapped.forEach(e => console.warn(`  - "${e.name}" (${e.sinner})`));
}
